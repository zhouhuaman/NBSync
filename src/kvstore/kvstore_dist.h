...
class KVStoreDist : public KVStoreLocal {
    ...
    if (IsWorkerNode()) {
      ...
      ps_worker_ = new ps::KVWorker<char>(0, new_customer_id);
        ...
        ps_worker_->set_request_handle(
                std::bind(&KVStoreDist::DataHandleEx, this, _1, _2, _3));
        ps_worker_->set_nbsync_request_handle(
                std::bind(&KVStoreDist::NBSyncEx, this, _1, _2, _3));
        exec_thread_ = std::unique_ptr<std::thread>(
                new std::thread(&KVStoreDist::Exec, this));
        //huaman
 ...
 private:
  ...
  void InitImpl(const std::vector<int>& keys,
    ...
    if (get_rank() == 0 && this->ps_worker_->get_customer()->customer_id() == 0) { //huaman
      //if (this->ps_worker_->get_customer()->customer_id() == 0) { //huaman
...
//huaman
    bool PullImplIsGlobalModel() override {
        if(!(key_request.find(global_sync_num) == key_request.end()) && key_request[global_sync_num] == 0){
            return true;
        }else{
            return false;
        }
...
//huaman
  ...
  static DataHandleType DepairDataHandleType(int cmd) {
    ...
    void DataHandleEx(const ps::KVMeta& req_meta,
        ...
        switch (type.requestType) {
            ...
            case RequestType::kDefaultPushPull:
                DataHandleDefault(type, req_meta, req_data, worker);
                break;
        }
    }
    void NBSyncEx(const ps::KVMeta& req_meta,
                      const ps::KVPairs<char>& req_data,
                      ps::KVWorker<char>* worker) {
        DataHandleType type = DepairDataHandleType(req_meta.cmd);
        switch (type.requestType) {
            ...
            case RequestType::kCompressedPushPull:
                ...
                break;
            case RequestType::kDefaultPushPull:
                NBSyncHandle(type, req_meta, req_data, worker);
                break;
        }
    }
    void Exec() {
    ...
    void DataHandleDefault(const DataHandleType type, const ps::KVMeta& req_meta,
        ...
        if (req_meta.push) {
            ...
            if (stored.is_none()) {
                ...
                ApplyUpdates(type, key, &updates, worker);
            }
        } else {
            DefaultStorageResponse(type, key, req_meta, req_data, worker);
        }
    }
    void NBSyncHandle(const DataHandleType type, const ps::KVMeta& req_meta,
                           const ps::KVPairs<char> &req_data,
                           ps::KVWorker<char>* worker) {
        ...
        int key = DecodeKey(req_data.keys[0]);
        auto& global_stored = global_store_[key];
        auto& stored = store_[key];
        auto& global_stored_bk = global_store_bk_[key];
        // to do: send store and copy req_data to store

        //nbsync_exec_.Exec([this, key, &req_meta, &req_data, &stored](){

            //NBSync(key, req_meta, req_data, stored, worker);
        //});
        //todo:把req_data重的数据复制到store的中。
        size_t ds[] = {(size_t) req_data.lens[0] / mshadow::mshadow_sizeof(type.dtype)};
        TShape dshape(ds, ds + 1);
        TBlob recv_blob;
        MSHADOW_REAL_TYPE_SWITCH(type.dtype, DType, {
        ...
        if (stored.is_none()) {
            ...
            CopyFromTo(recved, &stored, 0);
            stored.WaitToRead();
            if (global_stored_bk.is_none()) {
                // initialization
                global_stored_bk = NDArray(dshape, Context(), false,
                                 has_multi_precision_copy(type) ? mshadow::kFloat32 : type.dtype);
                CopyFromTo(stored, &global_stored_bk, 0);
                global_stored_bk.WaitToRead();
            }
            key_number++;
            std::cout << "key_number=" << key_number << std::endl;
        }else{
            if (global_stored.is_none()) {
                // initialization
                global_stored = NDArray(dshape, Context(), false,
                                        has_multi_precision_copy(type) ? mshadow::kFloat32 : type.dtype);
            }
            CopyFromTo(recved, &global_stored, 0);
            global_stored.WaitToRead();
            global_sync_completed[key]=true;
            if(key_sync.find(key) == key_sync.end()) key_sync[key] = 0;
            key_sync[key]++;
            if(key_response.find(key_sync[key]) == key_response.end()) key_response[key_sync[key]] = 0;
            key_response[key_sync[key]]++;
            //std::cout << key << "," << key_sync[key] << "," << key_response[key_sync[key]] << std::endl;
            if(key_response[key_sync[key]] == key_number){
                key_request[key_sync[key]] = key_number;
            }
        }
    }
    void NBSync(int key, const ps::KVMeta& req_meta,
                const ps::KVPairs<char> &req_data, const NDArray& stored, ps::KVWorker<char>* worker){
        ps::KVPairs<char> global_request;

        auto len = stored.shape().Size() * mshadow::mshadow_sizeof(stored.dtype());
        global_request.keys = req_data.keys;
        global_request.lens = {len};
        // TODO(mli) try to remove this CopyFrom
        global_request.vals.CopyFrom(static_cast<const char*>(stored.data().dptr_), len);
        worker->Global_Request(req_meta, global_request);
    }

    inline void ApplyUpdates(const DataHandleType type, const int key,
                             UpdateBuf *update_buf, ps::KVWorker<char>* worker) {

            // let the main thread to execute updater_, which is necessary for python
            auto& stored = has_multi_precision_copy(type) ? store_realt_[key] : store_[key];
            auto& update =  update_buf->merged;
            if (updater_) {
    ...
    void DefaultStorageResponse(const DataHandleType type,
        ...
        NDArray& global_stored= global_store_[key];
        auto& global_stored_bk = global_store_bk_[key];
        NDArray& temp= temp_[key];
        auto &updates = update_buf_[key];

        if(temp.is_none()){
            temp = NDArray(stored.shape(), Context(), false,
                           has_multi_precision_copy(type) ? mshadow::kFloat32 : type.dtype);

        }
        if(global_stored.is_none()){
            CopyFromTo(stored - global_stored_bk, &temp, 0);
            temp.WaitToRead();
            global_pull[key] = 0;
            local_pull[key] = 0;
            global_sync_num = 1;
        }else{
            if(!(key_request.find(global_sync_num) == key_request.end()) && key == key_number - key_request[global_sync_num] && global_sync_completed[key]){
                CopyFromTo(stored - global_stored_bk, &temp, 0);
                temp.WaitToRead();
                CopyFromTo(global_stored, &stored, 0);
                stored.WaitToRead();
                CopyFromTo(global_stored, &global_stored_bk, 0);
                global_stored_bk.WaitToRead();
            }
        }
        ...
        worker->Response(req_meta, response);

        if(global_stored.is_none()){
            global_stored = NDArray(stored.shape(), Context(), false,
                                    has_multi_precision_copy(type) ? mshadow::kFloat32 : type.dtype);
            exec_.Exec([this, key, &req_meta, &req_data, &temp, &worker](){
                NBSync(key, req_meta, req_data, temp, worker);
            });
        }else{
            if(!(key_request.find(global_sync_num) == key_request.end()) && key_request[global_sync_num] == 0){
                global_sync_num++;
            }
           // std::cout << "key = " << key << ":" << int(!(key_request.find(global_sync_num) == key_request.end())) << key_number - key_request[global_sync_num] << int(global_sync_completed[key])<< std::endl;
            if(!(key_request.find(global_sync_num) == key_request.end()) && key == key_number - key_request[global_sync_num] && global_sync_completed[key]){
                global_sync_completed[key]=false;
                exec_.Exec([this, key, &req_meta, &req_data, &temp, &worker](){
                    NBSync(key, req_meta, req_data, temp, worker);
                });
                key_request[global_sync_num]--;
                global_pull[key]++;
                local_pull[key] = 0;
        ...
        local_pull[key]++;
        //std::cout << "global_pull[" << key << "] = " << global_pull[key] << "local_pull[" << key << "] = "  << local_pull[key]<< std::endl;
    }
   ...
   */
    ...
    std::unordered_map<int, NDArray> global_store_;
    std::unordered_map<int, NDArray> global_store_bk_;
    std::unordered_map<int, NDArray> temp_;
    std::unordered_map<int, NDArray> store_realt_;

    std::unordered_map<int, UpdateBuf> update_buf_;
    bool multi_precision_;
    Executor exec_;
    std::unordered_map<int,bool> global_sync_completed;
    std::unordered_map<int,int> local_pull;
    std::unordered_map<int,int> global_pull;
    std::unique_ptr<std::thread> exec_thread_;
    std::unordered_map<int,int> key_request;
    std::unordered_map<int,int> key_response;
    std::unordered_map<int,int> key_sync;
    int global_sync_num = 1;
    int key_number=0;
