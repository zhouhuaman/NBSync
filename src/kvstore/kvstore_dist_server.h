 ...
 private:
  ...
  inline void ApplyUpdates(const DataHandleType type, const int key,
    ...
    if (!sync_mode_ || update_buf->request.size() == (size_t) ps::NumWorkers()) {
      ...
      stored = stored + update/(size_t) ps::NumWorkers();
      //nbsync_derectadd
      ...
      if (updater_) {
        exec_.Exec([this, key, &update, &stored](){
          CHECK(updater_);
          updater_(key, update/(size_t) ps::NumWorkers(), &stored);
        });
        //CopyFromTo(update_buf->merged / (size_t) ps::NumWorkers(), &stored); //huaman
      } else {
      ...
      */
      //CopyFromTo(update_buf->merged/(size_t) ps::NumWorkers(), &stored);
      if (log_verbose_)  {
        LOG(INFO) << "sent response to " << update_buf->request.size() << " workers";
      }
      //huaman
      ps::KVPairs<char> req_data;
      req_data.keys.push_back(key);
      //huaman
      for (const auto& req : update_buf->request) {
        server->Response(req, req_data);  //huaman
      }
  ...
  void DataHandleDefault(const DataHandleType type, const ps::KVMeta& req_meta,
    ...
    if (req_meta.push) {
      ...
      if (stored.is_none()) {
        ...
        server->Response(req_meta, req_data); //huaman
        if (has_multi_precision_copy(type)) {
