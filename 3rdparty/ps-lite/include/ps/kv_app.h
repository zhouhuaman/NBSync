...
class KVWorker : public SimpleApp {
 public:
  ...
  explicit KVWorker(int app_id, int customer_id) : SimpleApp() {
    ...
    obj_ = new Customer(app_id, customer_id, std::bind(&KVWorker<Val>::Global_Process, this, _1)); //huaman
      recv_thread_ = std::unique_ptr<std::thread>(new std::thread(&KVWorker::NBSync_Receiving, this));
      global_request_thread_ = std::unique_ptr<std::thread>(new std::thread(&KVWorker::Global_Request_Receiving, this));
  }
  ...
  int ZPush(const SArray<Key>& keys,
            ...
            int cmd = 0,
            const Callback& cb = nullptr) { //huaman
  ...
  //huaman
    ...
    void Global_Request(const KVMeta& req, const KVPairs<Val>& res);
    /**
    ...
    void set_nbsync_request_handle(const ReqHandle& nbsync_request_handle) {
        CHECK(nbsync_request_handle) << "invalid request handle";
        nbsync_request_handle_ = nbsync_request_handle;
    }
    /** \brief request handle */
    ReqHandle request_handle_;
    ReqHandle nbsync_request_handle_;
 ...
 private:
  ...
  //huaman
  void Local_Send(int timestamp, bool push, int cmd, const KVPairs<Val>& kvs);
    void Local_Process(const Message& msg);
    void Global_Process(const Message& msg);
    void NBSync_Receiving();
    void Global_Request_Receiving();
    ThreadsafeQueue<Message> recv_queue_;
    ThreadsafeQueue<Message> global_request_queue_;
    std::unique_ptr<std::thread> recv_thread_;
    std::unique_ptr<std::thread> global_request_thread_;
    std::unordered_map<int,bool> is_init_completed;
  //huaman
};
...
//huaman
    template <typename Val>
    void KVWorker<Val>::Local_Process(const Message& msg) {
        KVMeta meta;
        meta.cmd       = msg.meta.head;
        meta.push      = msg.meta.push;
        meta.sender    = msg.meta.sender;
        meta.receiver    = msg.meta.recver;
    ...
    template <typename Val>
    void KVWorker<Val>::NBSync_Receiving() {
        while (true) {
            Message recv;
    ...
    template <typename Val>
    void KVWorker<Val>::Global_Request_Receiving() {
        while (true) {
            Message recv;
            global_request_queue_.WaitAndPop(&recv);
            Postoffice::Get()->van()->Send(recv);
    ...
    void KVWorker<Val>::Response(const KVMeta& req, const KVPairs<Val>& res) {
        ...
        Process(msg);
        obj_->AddResponse(msg.meta.timestamp,1);
    }
    template <typename Val>
    void KVWorker<Val>::Global_Request(const KVMeta& req, const KVPairs<Val>& res) {
        Message msg;
        msg.meta.app_id = obj_->app_id();
        msg.meta.customer_id = req.customer_id;
        ...
        msg.meta.head        = req.cmd;
        msg.meta.timestamp   = req.timestamp;
        msg.meta.recver      = req.receiver;      //huaman
        if (res.keys.size()) {
            msg.AddData(res.keys);
            msg.AddData(res.vals);
            if (res.lens.size()) {
        ...
        global_request_queue_.Push(msg);  //to ps

    }
    template <typename Val>
    void KVWorker<Val>::Global_Process(const Message& msg) {
        ...
        if (!msg.meta.push && msg.data.size()) {  //pull response
            KVMeta meta;
            meta.cmd       = msg.meta.head;
            meta.push      = msg.meta.push;
            meta.sender    = msg.meta.sender;
            ...
            CHECK(nbsync_request_handle_);
            nbsync_request_handle_(meta, data, this);
            if(!is_init_completed[key]){
                is_init_completed[key] = true;
                //obj_->AddResponse(msg.meta.timestamp,1);
                Process(msg);
            }
        }else{ //push response，start pull request
            if(!is_init_completed[key]){
                //obj_->AddResponse(msg.meta.timestamp,1);
                Process(msg);
            }else{
                Message pullmsg;
                pullmsg.meta.app_id = obj_->app_id();
                pullmsg.meta.customer_id = obj_->customer_id();
                pullmsg.meta.request     = true;
                pullmsg.meta.push        = false;
                pullmsg.meta.head        = msg.meta.head;
                pullmsg.meta.timestamp   = msg.meta.timestamp;
                pullmsg.meta.recver      = msg.meta.sender;
                if (msg.data[0].size()) {
                    pullmsg.AddData(msg.data[0]);
                    pullmsg.AddData(msg.data[1]);
                }
                Postoffice::Get()->van()->Send(pullmsg);  //to ps
            }
...
void KVWorker<Val>::Send(int timestamp, bool push, int cmd, const KVPairs<Val>& kvs) {
  ...
  for (size_t i = 0; i < sliced.size(); ++i) {
    ...
    Postoffice::Get()->van()->Send(msg);  //to ps
  }
}
//huaman
    template <typename Val>
    void KVWorker<Val>::Local_Send(int timestamp, bool push, int cmd, const KVPairs<Val>& kvs) {
        // slice the message
        SlicedKVs sliced;
        slicer_(kvs, Postoffice::Get()->GetServerKeyRanges(), &sliced);
        ...
        for (size_t i = 0; i < sliced.size(); ++i) {
            ...
            int key = kvs.keys[0];
            if(is_init_completed.find(key) == is_init_completed.end()){
                is_init_completed[key] = false;
            }
            if(is_init_completed[key]){
                recv_queue_.Push(msg);  //to local
            }else{
                Postoffice::Get()->van()->Send(msg);  //to ps
            }
